    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition-shadow">
            <div class="flex items-center justify-between mb-3">
                ...
                <div class="w-9 h-9 {{ $stockBajo > 0 ? 'bg-amber-50' : 'bg-slate-50' }} rounded-lg flex items-center justify-center shrink-0">
                    <i class="fas fa-triangle-exclamation {{ $stockBajo > 0 ? 'text-amber-500' : 'text-slate-300' }} text-sm"></i>
                </div>
            </div>
            <p class="text-2xl font-bold {{ $stockBajo > 0 ? 'text-amber-600' : 'text-slate-800' }}">{{ $stockBajo }}</p>
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition-shadow">
            ...
            <p class="text-2xl font-bold text-slate-800">S/ {{ number_format($ventasHoy, 2) }}</p>
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition-shadow">
            ...
            <p class="text-2xl font-bold text-slate-800">S/ {{ number_format($ventasMes, 2) }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div class="lg:col-span-2 bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="flex items-center justify-between px-5 py-4 border-b border-slate-100">
                ...
                <span class="text-xs text-slate-400">Últimas {{ $ventasRecientes->count() }}</span>
            </div>
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50 transition-colors">
                            <td class="px-5 py-3 text-xs text-slate-400 font-mono">#{{ $venta->id }}</td>
                            <td class="px-4 py-3">
                                <p class="font-medium text-slate-700 text-xs">
                                    {{ $venta->customer_name ?: 'Cliente general' }}
                                </p>
                                @if($venta->customer_document)
                                <p class="text-slate-400 text-xs font-mono">{{ $venta->customer_document }}</p>
                                @endif
                            </td>
                            <td class="px-4 py-3">
